...
if ( is_multisite() && is_plugin_active_for_network( 'wordpress-ecommerce/marketpress.php' ) ) {
	...
	class MarketPress_Global_Product_List extends WP_Widget {
		...
		function form( $instance ) {
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'custom_text' ) ); ?>"><?php _e( 'Custom Text:', 'mp' ) ?><br />
					<textarea class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'custom_text' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'custom_text' ) ); ?>"><?php echo esc_attr( $custom_text ); ?></textarea></label>
			</p>

			<h3><?php _e( 'List Settings', 'mp' ); ?></h3>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'per_page' ) ); ?>"><?php _e( 'Number of Products:', 'mp' ) ?> <input id="<?php echo esc_attr( $this->get_field_id( 'per_page' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'per_page' ) ); ?>" type="text" size="3" value="<?php echo esc_attr( $per_page ); ?>" /></label><br />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'order_by' ) ); ?>"><?php _e( 'Order Products By:', 'mp' ) ?><br />
					...
					<label><input value="DESC" name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>" type="radio"<?php checked( $order, 'DESC' ) ?> /> <?php _e( 'Descending', 'mp' ) ?></label>
					<label><input value="ASC" name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>" type="radio"<?php checked( $order, 'ASC' ) ?> /> <?php _e( 'Ascending', 'mp' ) ?></label>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php _e( 'Limit To Product Category:', 'mp' ) ?></label><br />
				<input id="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'category' ) ); ?>" type="text" value="<?php echo esc_attr( isset( $category ) ? $category : ''  ); ?>" title="<?php _e( 'Enter the Slug', 'mp' ); ?>" class="widefat" />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'tag' ) ); ?>"><?php _e( 'Limit To Product Tag:', 'mp' ) ?></label><br />
				<input id="<?php echo esc_attr( $this->get_field_id( 'tag' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'tag' ) ); ?>" type="text" value="<?php echo esc_attr( isset( $tag ) ? $tag : ''  ); ?>" title="<?php _e( 'Enter the Slug', 'mp' ); ?>" class="widefat" />
			</p>
			...
			<p>
				<input type="checkbox" class="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'show_price' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_price' ) ); ?>"<?php checked( $show_price ); ?> />
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_price' ) ); ?>"><?php _e( 'Show Price', 'mp' ); ?></label>
			</p>
